<script lang="ts">
	...
	onMount(async () => {
		...
		updatedArray = data.records.map((record) => ({
			...record,
	...
	const cryptoRandomNumber = (min: number, max: number) =>
		Math.floor(cryptoRandom() * (max - min + 1) + min);
	let colors = ['#8C3061', '#5865f2', '#1A3636', '#49243E', '#A76F6F'];

	dayjs.extend(relativeTime);
	let job_content: HTMLElement | undefined = $state();
	...
	let captureFilters = $state({ time: '0', postition: '', location_type: '', category: '' });
	let records = $state(data.records);
	let filteredData = $state([]);
	let isFiltered = $state(false);

	function filterByArray(data: [], array: [], data_property: string) {
		if (array === '') {
			return data;
	...
	function filterByTime(data: [], time_in_day: string, data_property: string) {
		if (Number(time_in_day) === 0) {
			return data;
		}
		...
		data = data.filter((record) => {
			...
			let record_created_time = new Date(record[data_property]).getTime();
	...
	function applyFilter() {
		isFiltered = true;
		filteredData = filterByTime(updatedArray, captureFilters.time, 'created');
	...
	async function handleBookmark(id: string) {
		console.log(id);
		let local_id = await add_Job_Post(id);
		let result = await local_db.posts.toArray();
		let jobIds = result.map((item) => item.job_post_id);
		console.log(jobIds);

		// let added = await local_db.posts.add({job_post_id: id});
		// console.log("Added data: ", added);
		// let [{id: result}] = await local_db.posts.where('job_post_id').equals(id).toArray();
		// console.log("Query result: ", result);
		// await local_db.posts.delete(4);
		// Dexie.delete('urban-job').then(() => console.log("database deleted successfully"))
	}
</script>

<div class="bg-primary text-center py-3 px-4">
	<p>
		🛠️ <strong>Under Construction!</strong> Filtering, Mobile responsive & Other pages will be available
		within a couple of weeks.
	</p>
</div>
<header class="bg-charcoal-200 text-silver-200 h-screen">
	<section
		class="xl:container mx-auto h-full flex flex-col justify-between max-[420px]:justify-start"
	>
		<nav
			class=" flex items-center justify-between border-b border-charcoal-100 max-[420px]:justify-center"
		>
			<div class="py-6">
				<svg
					width="99"
			...
			<div class=" text-base/4 font-normal text-silver-100 uppercase max-[420px]:hidden">
				...
				<a
					href="/"
					class=" px-12 py-6 border-l border-charcoal-100 text-primary hover:text-[#438F00]"
					>Give a Gift</a
				>
			</div>
		</nav>

		<div class="flex flex-col items-center gap-6 max-[420px]:pt-28">
			<h1 class=" text-6xl/snug text-silver-100 max-w-xl text-center mx-auto max-[420px]:text-4xl">
				Land Your Dream Tech Job. Start Searching Now.
			</h1>
			<a
				onclick={handleExplore}
				href="/"
				class=" text-silver-100 border border-charcoal-100 py-4 px-12 w-fit hover:bg-[#080808]"
				>EXPLORE</a
			>
		</div>

		<section class=" w-full border-y border-charcoal-100 flex justify-between max-[420px]:hidden">
			<div class=" flex gap-6 items-center py-9 px-12 border-r border-charcoal-100">
				<a
					href="https://www.facebook.com/profile.php?id=61562502044091"
					target="_blank"
				...
				<a href="https://www.linkedin.com/company/urbanjob/" target="_blank" class="cursor-pointer">
					<svg
						width="20"
						height="20"
						viewBox="0 0 20 20"
				...
				<a href="https://www.instagram.com/urban_job_bd/" target="_blank" class="cursor-pointer">
					<svg
						width="20"
						height="20"
						viewBox="0 0 20 20"
				...
				<a href="https://x.com/urban_job_bd" target="_blank" class="cursor-pointer">
					<svg
						width="20"
						height="20"
						viewBox="0 0 20 20"
					...
					>
						<path
							d="M15.2719 1.58643H18.0831L11.9414 8.606L19.1667 18.1581H13.5094L9.07836 12.3648L4.00829 18.1581H1.19536L7.76453 10.6498L0.833336 1.58643H6.63426L10.6395 6.8817L15.2719 1.58643ZM14.2853 16.4754H15.843L5.78783 3.1807H4.11622L14.2853 16.4754Z"
							fill="#4D4D4D"
						/>
					</svg>
				</a>
			...
			<div class=" flex items-center border-l border-charcoal-100 text-charcoal-50">
				<h2 class=" px-8 py-9 uppercase">Mehedi Hasan</h2>
				<h2 class=" px-12 py-9 border-l border-charcoal-100">
					34.07062188658319, <br />-84.27424250862218
				</h2>
			</div>
		</section>
	</section>
...
<main bind:this={job_content} class=" bg-charcoal-200 text-silver-200 relative">
	<div class="xl:container mx-auto flex max-[420px]:flex-col items-start">
		<aside
			class="py-10 border-r border-charcoal-100 min-w-96 sticky top-0 min-h-min max-[420px]:hidden"
		>
			<div class=" pb-4 px-10 border-b border-charcoal-100">
				<h3 class=" text-xl/5 font-medium">Filters</h3>
			</div>
			<div class=" py-4 px-10 border-b border-charcoal-100">
				<h3 class=" text-xl/5 font-medium">Date Posted</h3>
				<select
					bind:value={captureFilters.time}
					class="cursor-pointer bg-charcoal-200 px-4 py-2 border border-charcoal-100 mt-4 w-full focus:border-primary focus:ring-primary"
					name=""
					id=""
				>
			...
			<div class=" py-4 px-10 border-b border-charcoal-100">
				<h3 class=" text-xl/5 font-medium">Job Position</h3>
				<div class=" grid grid-cols-2 gap-y-3 checkbox-container mt-4 even:justify-self-end">
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="junior"
							><input
								bind:group={captureFilters.postition}
								value="junior"
								type="checkbox"
								name="junior"
								id=""
								class="cursor-pointer form-checkbox bg-charcoal-200 border border-charcoal-100 rounded-[3px] text-primary focus:ring-charcoal-200 focus:ring-offset-charcoal-200 focus:bg-[url('data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%2016%2016%22%20fill%3D%22black%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22M12.207%204.793a1%201%200%20010%201.414l-5%205a1%201%200%2001-1.414%200l-2-2a1%201%200%20011.414-1.414L6.5%209.086l4.293-4.293a1%201%200%20011.414%200z%22%2F%3E%3C%2Fsvg%3E')] checked:bg-[url('data:image/svg+xml;base64,')]"
							/> JUNIOR</label
						>
					</div>
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="intern"
							><input
								bind:group={captureFilters.postition}
								value="intern"
								type="checkbox"
								name="intern"
								id=""
								class="cursor-pointer form-checkbox bg-charcoal-200 border border-charcoal-100 rounded-[3px] text-primary focus:ring-charcoal-200 focus:ring-offset-charcoal-200 focus:bg-[url('data:image/svg+xml;base64,')] checked:bg-[url('data:image/svg+xml;base64,')]"
							/> INTERN</label
						>
					</div>
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="senior"
							><input
								bind:group={captureFilters.postition}
								value="senior"
								type="checkbox"
			...
			<div class=" py-4 px-10">
				<h3 class=" text-xl/5 font-medium">Location</h3>
				<div class=" grid grid-cols-2 gap-y-3 checkbox-container mt-4 even:justify-self-end">
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="on-site"
							><input
								bind:group={captureFilters.location_type}
								value="on-site"
								type="checkbox"
								name="on-site"
								id=""
					...
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="remote"
							><input
								bind:group={captureFilters.location_type}
								value="remote"
								type="checkbox"
					...
					<div>
						<label class="flex gap-2 items-center text-sm/4 font-normal" for="hybrid"
							><input
								bind:group={captureFilters.location_type}
								value="hybrid"
								type="checkbox"
			...
			<!-- <div class=" py-4 px-10">
                <h3 class=" text-xl/5 font-medium ">Categories</h3>
                <div class=" grid grid-cols-2 gap-y-3 checkbox-container mt-4 even:justify-self-end">
                    <div><label class="flex gap-2 items-center text-sm/4 font-normal" for="web developer"><input bind:group={captureFilters.category} value="web developer" type="checkbox" name="web developer" id="" class="cursor-pointer form-checkbox bg-charcoal-200 border border-charcoal-100 rounded-[3px] text-primary focus:ring-charcoal-200 focus:ring-offset-charcoal-200 focus:bg-[url('data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%2016%2016%22%20fill%3D%22black%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22M12.207%204.793a1%201%200%20010%201.414l-5%205a1%201%200%2001-1.414%200l-2-2a1%201%200%20011.414-1.414L6.5%209.086l4.293-4.293a1%201%200%20011.414%200z%22%2F%3E%3C%2Fsvg%3E')] checked:bg-[url('data:image/svg+xml;base64,')] ">  WEB DEVELOPER</label></div>
                    <div><label class="flex gap-2 items-center text-sm/4 font-normal" for="ui/ux design"><input bind:group={captureFilters.category} value="ui/ux designer" type="checkbox" name="ui/ux designer" id="" class="cursor-pointer form-checkbox bg-charcoal-200 border border-charcoal-100 rounded-[3px] text-primary focus:ring-charcoal-200 focus:ring-offset-charcoal-200 focus:bg-[url('data:image/svg+xml;base64,')] checked:bg-[url('data:image/svg+xml;base64,')] ">  UI/UX Design</label></div>
                </div>
            </div> -->
			...
			<div class="px-10 mt-6">
				<button
					onclick={applyFilter}
					class="bg-primary hover:bg-[#65DA00] w-full py-4 uppercase text-lg/5 font-medium text-charcoal-200"
					>Apply Filter</button
				>
			</div>
		</aside>
		<section class="regular max-[420px]:w-full w-full">
					...
					<div>
						...
						>
							<path
								fill-rule="evenodd"
								clip-rule="evenodd"
								d="M259.193 75.5259C248.828 102.433 190.76 63.6678 175.366 88.0502C156.161 118.468 211.26 168.992 184.798 193.361C163.731 212.762 149.432 146.6 122.567 136.673C99.6973 128.223 72.3116 156.368 51.2847 144.028C31.2398 132.265 15.0576 100.526 26.579 80.3421C41.4036 54.3717 81.6274 60.9459 107.384 45.751C114.216 41.7202 113.444 31.0895 119.243 25.6759C125.713 19.635 133.343 12.2648 142.134 13.3077C151.017 14.3616 153.6 27.2173 161.828 30.728C172.465 35.2666 184.896 31.2026 195.403 36.0365C218.591 46.7056 268.369 51.7081 259.193 75.5259Z"
								fill="#101010"
							/>
							<path
								d="M124.167 27.6943C90.205 27.6943 62.5983 55.301 62.5983 89.2627C62.5983 123.224 90.205 150.831 124.167 150.831C158.128 150.831 185.735 123.224 185.735 89.2627C185.735 55.301 158.128 27.6943 124.167 27.6943ZM124.167 104.638H103.667C100.865 104.638 98.5417 102.314 98.5417 99.5127C98.5417 96.711 100.865 94.3877 103.667 94.3877H124.167C126.968 94.3877 129.292 96.711 129.292 99.5127C129.292 102.314 126.968 104.638 124.167 104.638ZM144.667 84.1377H103.667C100.865 84.1377 98.5417 81.8143 98.5417 79.0127C98.5417 76.211 100.865 73.8877 103.667 73.8877H144.667C147.468 73.8877 149.792 76.211 149.792 79.0127C149.792 81.8143 147.468 84.1377 144.667 84.1377Z"
								fill="white"
							/>
							<path
								d="M199.265 143.587C197.01 139.419 192.227 137.096 185.804 137.096C180.952 137.096 176.784 139.077 174.324 142.494C171.864 145.911 171.317 150.489 172.82 155.067C175.759 163.951 180.884 165.932 183.685 166.274C184.095 166.342 184.505 166.342 184.984 166.342C187.99 166.342 192.637 165.044 197.147 158.279C200.769 153.017 201.452 147.756 199.265 143.587Z"
								fill="white"
							/>
							...
							<rect
								...
								height="9"
								rx="0.5"
								transform="rotate(-90 31 24)"
								fill="white"
							/>
							<ellipse cx="137.5" cy="193.5" rx="121.5" ry="5.5" fill="#101010" />
						</svg>
					</div>
					<h3 class="text-4xl/9 font-semibold">No Jobs Found</h3>
				</section>
			{:else if filteredData.length > 0}
				{#each filteredData as { id, title, company_logo, company_name, job_position, location, location_type, created, salary_range, why_join_us, isBookmarked }, i}
					{@const colorCode = colors[cryptoRandomNumber(0, colors.length - 1)]}
					<a data-sveltekit-preload-data="tap" href={`/job/${id}`}>
						<article
							class="flex flex-col gap-6 py-12 px-14 border-b text-silver-100 border-charcoal-100 hover:bg-[#080808] cursor-pointer max-[420px]:px-4"
						>
							<header
								class="flex justify-between items-center max-[420px]:flex-col max-[420px]:items-start max-[420px]:gap-4"
							>
								<div class="flex items-center gap-4 max-[420px]:flex-col max-[420px]:items-start">
									...
									{:else}
										<div
											...
											class="w-14 h-14 rounded-sm inline-flex items-center justify-center text-2xl/6 font-semibold font-mono"
										>
									...
									<div>
										...
										<p class="font-medium text-xs flex gap-3">
											{company_name} <span>&bull;</span>
											{capitalizeFirstLetter(job_position)} <span>&bull;</span> BDT {salary_range
												.range.min / 1000}-{salary_range.range.max / 1000}k
										</p>
									</div>
								</div>
								<div class="flex gap-9 items-center">
									<div class=" text-right max-[420px]:text-left">
										<p class="font-medium text-base/6 flex gap-2 items-center mb-1">
											<svg
												width="16"
												height="16"
												viewBox="0 0 16 16"
											...
											>
												...
												<path
													...
													stroke-linejoin="round"
												/>
											</svg> <span>{capitalizeFirstLetter(location_type)}, {location}</span>
										</p>
										<p class="text-charcoal-50 font-normal text-sm/4">
									...
									{#if isBookmarked}
										<button
											onclick={async (e: MouseEvent) => {
												...
												await delete_Job_Post(local_indexDB_id);
												filteredData[i].isBookmarked = false;
											}}
									...
									{:else}
										<button
											onclick={async (e: MouseEvent) => {
												...
												filteredData[i].isBookmarked = true;
											}}
			...
			{:else if updatedArray}
				{#each updatedArray as { id, title, company_logo, company_name, job_position, location, location_type, created, salary_range, why_join_us, isBookmarked }, i}
					{@const colorCode = colors[cryptoRandomNumber(0, colors.length - 1)]}
					<a data-sveltekit-preload-data="tap" href={`/job/${id}`}>
						<article
							class="flex flex-col gap-6 py-12 px-14 border-b text-silver-100 border-charcoal-100 hover:bg-[#080808] cursor-pointer max-[420px]:px-4"
									...
									{#if isBookmarked}
										<button
											onclick={async (e: MouseEvent) => {
												...
												let local_indexDB_id = await get_Local_ID_By_Job_Post(id);
												await delete_Job_Post(local_indexDB_id);
												updatedArray[i].isBookmarked = false;
											}}
											class="w-8 h-8 inline-flex justify-center items-center rounded bg-[#0d0d0d] hover:bg-charcoal-100"
										>
											<svg
									...
									{:else}
										<button
											onclick={async (e: MouseEvent) => {
												...
												e.stopPropagation();
												await add_Job_Post(id);
												updatedArray[i].isBookmarked = true;
											}}
											class="w-8 h-8 inline-flex justify-center items-center border rounded border-charcoal-100 hover:bg-charcoal-100"
										>
											<svg
...
<style>
	...
	.checkbox-container div:nth-child(even) label {
