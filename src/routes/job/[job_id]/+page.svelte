...
<main
	class="bg-charcoal-200 text-silver-100 flex gap-24 items-start 2xl:px-80 px-48 py-10 max-[420px]:flex-col max-[420px]:px-5"
>
	<section>
		<article>
			...
			{/if}
			<div
				class="flex justify-between items-start mt-6 mb-3 max-[420px]:flex-col max-[420px]:gap-3"
			>
				<h1 class="font-bold text-xl max-w-64 text-silver-200">
					{data.records.title} at {data.records.company_name}
				</h1>
				<p class="font-medium text-base/6 flex gap-2 items-center mb-1">
					...
					<span>{capitalizeFirstLetter(data.records.location_type)}, {data.records.location}</span>
				</p>
			</div>
			<div class="paragraph">
	...
	<aside class="min-w-80 sticky top-4 max-[420px]:w-full">
		<div class=" mb-6 flex gap-4 items-center">
			<span class="px-2 py-[6px] bg-silver-100 text-charcoal-200 rounded align-middle"
				>{data.records?.experience_range?.range?.min}-{data.records?.experience_range?.range?.max} years</span
			>
			<span>&bull;</span>
			<span class="px-2 py-[6px] bg-silver-100 text-charcoal-200 rounded align-middle"
				>{capitalizeFirstLetter(data.records.job_position)}</span
			>
			<span>&bull;</span>
			<span class="px-2 py-[6px] bg-silver-100 text-charcoal-200 rounded align-middle"
				>{data.records?.salary_range?.range?.min / 1000}-{data.records?.salary_range?.range?.max /
					1000}k</span
			>
		</div>
		<button
			...
			class="mb-4 h-12 bg-primary hover:bg-[#65DA00] w-full py-4 uppercase text-lg/5 font-medium text-charcoal-200"
			>Apply Now</button
		>
